...
/*
Animates some GameObjects. Requires the gameobject to have the number of frames on the spritesheet,
and the framerate.
 */
...
public class Animation {
    ...
    //Sets the bitmap, frame dimensions, fps, frame count, and pixelsperMetre dependingo on the viewport
    //derived from the specific phone screen.
    ...
    //Gets the current frame depending on the time and if the GameObject is moving
    public Rect getCurrentFrame(long time, float xVelocity, float yVelocity, boolean moves){
        ...
        //If y velocity is not 0, then animate the object as well
        if(yVelocity != 0){
            if (time > frameTicker + framePeriod) {
                frameTicker = time;
                currentFrame++;
